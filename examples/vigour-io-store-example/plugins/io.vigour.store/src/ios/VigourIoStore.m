...
- (void)jsEmit:(NSString *)eventType withMessage:(NSDictionary *)message
{
    NSString* jsonString = @"";
    
    if(message)
    {
        NSData* jsonData = [NSJSONSerialization dataWithJSONObject:message options:1 error:nil];
        jsonString = [[NSString alloc] initWithBytes:[jsonData bytes] length:[jsonData length] encoding:NSUTF8StringEncoding];
    }
    
//    NSString *js = [NSString stringWithFormat:@"var em = new Emitter();alert(1);em.emit('%@', %@);", eventType, jsonString];
...
- (void)fetch:(CDVInvokedUrlCommand*)command
{
    ...
    [self jsEmit:@"fetch" withMessage:nil];
...
{
    NSDictionary *message = @{@"err":@"Transaction failed"};
    ...
    [self jsEmit:@"transactionFinished" withMessage:message];
