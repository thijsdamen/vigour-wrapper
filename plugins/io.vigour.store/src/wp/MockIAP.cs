...
namespace MockIAPLib
{
	public static class MockIAP
	{
		...
		public static void AddProductListing(string key, ProductListing productListing)
		{
			...
			// Add one minute to the expiration date
			var expDate = DateTimeOffset.Now.AddMinutes(1);
		...
		internal static string SimulatePurchase(string ProductId, bool includeReceipt)
		{
			...
			if (state != null && state.Value == false)
			{
				...
				Debug.WriteLine("You have already purchased this but not fulfiled it yet");
				OkToPurchase = false;
				//});
			}
			else
			{
				...
				if (listing.ProductType == ProductType.Durable && rs.EnumerateReceipts().ContainsKey(listing.ProductId) && DateTimeOffset.Now < CurrentApp.LicenseInformation.ProductLicenses[listing.ProductId].ExpirationDate)
				{
					//Deployment.Current.Dispatcher.BeginInvoke(() =>
					//	{
					//MessageBox.Show("You already purchased this durable");
					Debug.WriteLine("You already purchased this durable");
					OkToPurchase = false;
					//});
				}
			}
			...
			if (OkToPurchase)
			{
				...
				//Deployment.Current.Dispatcher.BeginInvoke(() =>
				//{
				//	result = MessageBox.Show(string.Format("Simulating purchase. Do you want to buy this item ({0})?", listing.Name), "Mock UI", MessageBoxButton.OKCancel);
